...
def convert_examples_to_features(item):
    ...
    lang = getattr(args, 'language', 'java')
    parser=parsers[lang]
    ...
    for url in [url1,url2]:
        if url not in cache:
            ...
            code_tokens,dfg=extract_dataflow(func,parser,lang)
...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_path='train', lcgs_features=None):
        ...
        data_dir = os.path.dirname(os.path.abspath(index_filename))
        jsonl_path = os.path.join(data_dir, 'data.jsonl')
        if not os.path.exists(jsonl_path):
            jsonl_path = os.path.join(os.path.dirname(data_dir), 'data.jsonl')
        if not os.path.exists(jsonl_path):
            raise FileNotFoundError(f"data.jsonl not found in {data_dir}")
        ...
        if 'valid' in file_path:
            ...
            sampled_indices = sorted(random.sample(indices, int(len(indices)*0.1)))
            data = [data[i] for i in sampled_indices]
            self.lcgs_cwj = [self.lcgs_cwj[i] for i in sampled_indices]
            self.lcgs_mcs = [self.lcgs_mcs[i] for i in sampled_indices]
...
def train(args, train_dataset, model, tokenizer):
    ...
    for idx in range(args.epochs):
        ...
        for step, batch in enumerate(bar):
            ...
            labels, cwj_batch, mcs_batch)=[x.to(args.device)  for x in batch]
            model.train()
            ce_loss,logits = model(inputs_ids_1,position_idx_1,attn_mask_1,inputs_ids_2,position_idx_2,attn_mask_2,labels)
            ...
            if args.lambda_param > 0:
                ...
                if mask.any():
                    ...
                    l2_loss = torch.tensor(0.0, device=labels.device)
...
def evaluate(args, model, tokenizer, eval_when_training=False):
    ...
    for batch in eval_dataloader:
        (inputs_ids_1,position_idx_1,attn_mask_1,
        inputs_ids_2,position_idx_2,attn_mask_2,
        labels, _, _)=[x.to(args.device)  for x in batch]
        with torch.no_grad():
...
def main():
    ...
    parser.add_argument("--language", default="java", type=str,
                        help="Programming language (java, python, c, etc). If unsupported, DFG is skipped.")
