...
class DAGEmbedding(nn.Module):
    def __init__(self, node_out_channels, layers):
        ...
        self.F_B = nn.ModuleList(
            [MLPBlock(3 * node_out_channels, node_out_channels, batch=True) for _ in range(layers)])
        self.F_1 = nn.ModuleList([MLPBlock(node_out_channels, node_out_channels, batch=True) for _ in range(layers)])
        self.F_2 = nn.ModuleList([MLPBlock(node_out_channels, node_out_channels, batch=True) for _ in range(layers)])
        self.F_3 = nn.ModuleList([MLPBlock(node_out_channels, node_out_channels, batch=True) for _ in range(layers)])
    ...
    def forward(self, x, term_walk_index, node_weight=None):
        ...
        if node_weight is None:
            node_weight = torch.ones(x.size(0), dtype=x.dtype, device=x.device)

        def weighted_scatter_mean(values, index, weights, dim_size):
            num = scatter_add(weights, index, dim=0, dim_size=dim_size) + 1e-9
            den = scatter_add(values * weights.unsqueeze(-1), index, dim=0, dim_size=dim_size)
            return den / (num.unsqueeze(-1))
        ...
        for i in range(self.K):
            ...
            m_T = weighted_scatter_mean(trans_T, index=term_walk_index[0], weights=node_weight[term_walk_index[0]], dim_size=N)

            trans_M = self.F_M[i](term_walk_feat)
            m_M = weighted_scatter_mean(trans_M, index=term_walk_index[1], weights=node_weight[term_walk_index[1]], dim_size=N)

            trans_B = self.F_B[i](term_walk_feat)
            m_B = weighted_scatter_mean(trans_B, index=term_walk_index[2], weights=node_weight[term_walk_index[2]], dim_size=N)
...
class PremiseSelectionModel(nn.Module):
    ...
    def forward(self, batch):
        ...
        h_s = self.dag_emb(h_s, batch.term_walk_index_s, getattr(batch, 'node_w_s', None))
        h_t = self.dag_emb(h_t, batch.term_walk_index_t, getattr(batch, 'node_w_t', None))
...
def _load_node_dict():
    ...
    candidates.append(os.path.join(os.path.dirname(os.path.abspath(__file__)), "dataset_TWGNN", "re_node_dict.pkl"))
    for p in candidates:
        try:
